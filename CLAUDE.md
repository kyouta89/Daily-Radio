...
- **`.github/workflows/verify-tts.yml` is temporary/disposable.** It's a one-shot `FORCE_RUN=true` run (cron 09:15 JST + `workflow_dispatch`) to validate multi-speaker TTS on a fresh daily quota. It is marked for deletion once the 2026-07-22 verification is done — remove it, don't build on it.
...
4. **`audio.js` — generateAudio(script, GEMINI_API_KEY, dir, variant, dateStr)**: synthesizes via **Gemini-TTS** (`generativelanguage` REST API) using `variant.voiceA/voiceB` and a style from `variant.mood.style`. `sendTTSRequest` is the shared core (rate-limit pacing, retries, 429 daily/per-minute triage, PCM extraction); the two callers differ only in the request body they build. Gemini returns 24kHz/16-bit PCM; all parts are concatenated then **encoded to MP3 once via ffmpeg**, saved to `output/radio_YYYY-MM-DD.mp3` (JST), mixed with BGM, and uploaded via `r2.js`. Two synthesis paths:
   - **Default — multi-speaker (`geminiSynthSection`).** `splitIntoSections` (exported, and covered by `test/split-sections.js`) groups the dialogue into ≤2500-char blocks **without breaking mid-utterance** — note the script has a blank line between every utterance, so you can't split on blank lines (that yields one section per utterance); it bundles utterances until the next would exceed the cap. Each block is one request carrying `multiSpeakerVoiceConfig` (2 speakers keyed by `HOST_A.name`/`HOST_B.name`), so an episode is **~7 requests instead of ~94**. 2500 chars ≈ ~8 min audio ≈ ~9k output tokens, safely under the TTS session's 32k-token ceiling (which is why the whole episode still can't be a single request). Speaker labels are normalized to a half-width colon so the API maps voices to labels reliably.
     - ⚠️ **Prosody caveat:** multi-speaker changes delivery vs. the old per-line path, which is why the `verify-tts.yml` one-shot workflow (below) exists to get a real listen before this is trusted for daily runs.
