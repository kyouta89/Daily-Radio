...
- ✅ **ラウドネス正規化 実施済み**。全19曲を `ffmpeg -vn -af loudnorm=I=-16:TP=-1.5:LRA=11`（＋アートワーク除去）で一括正規化し、各トラックを統合ラウドネス −16 LUFS 近辺（実測 −15.8〜−15.9 LUFS）に揃えた。日替わりで曲が変わっても音量差が出にくい。以後、新規トラックを `assets/bgm/` に追加する際は同じコマンドで正規化してから置くこと（`mixBGM` は volume=0.12 で敷くだけ）。
